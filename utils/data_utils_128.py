...
def load_sample_ids(data_split, splits_df, len_seq_in, len_seq_predict, regions, generate_pkl, years, path=''):
    ...
    if 0:
        if generate_pkl:
            ...
            idxs_r = read_samples_ids(path, data_split, regions)
        else:
            idxs_r = generate_sample_ids(data_split, splits_df, len_seq_in, len_seq_predict, regions)
    else:
        idxs_r = generate_sample_ids(data_split, splits_df, len_seq_in, len_seq_predict, regions, years)
...
def generate_and_cache_sequences(split, splits_df, len_seq_in, len_seq_predict, regions, path_name):
    ...
    for data_split in ['training', 'validation', 'test']:
        idxs = generate_sample_ids(data_split, splits_df, len_seq_in, len_seq_predict, regions, years)
...
def read_samples_ids(path, data_split, regions):
    ...
    with open(path, 'rb') as f:
        ...
        samples = loaded_dict[data_split]
...
def get_sequence(seq, root, data_split, region, product, bands, bidx, preprocess=None, swap_time_ch=False, ds=None):
    ...
    mask_seq = []
    for s in seq:
        prods, masks = get_file(s, root, data_split, region, product, bands, bidx, preprocess, ds)
...
def get_file(sample_id, root, data_split, region, product, bands, bidx, preprocess=None, ds=None):
    ...
    x = read_file(sample_id, ds, region, bidx)
...
def read_file(sample_id, ds, region, bidx):
