...
def load_model(Model, params, checkpoint_path=''):
    ...
    if checkpoint_path == '':
        ...
        model = Model.load_from_checkpoint(checkpoint_path, UNet_params=params['model'], config=p, strict=False)
    return model
...
def train(params, gpus, mode, checkpoint_path): 
    ...
    model = load_model(WeatherFusionNet_stage2, params, checkpoint_path)
    # ------------
...
def set_parser():
    ...
    parser.add_argument("-f", "--config_path", type=str, required=False, default='config_weatherfusionnet.yaml',
                        help="path to config-yaml")
    parser.add_argument("-g", "--gpus", type=int, nargs='+', required=False, default=[0,1],
    ...
    parser.add_argument("-n", "--name", type=str, required=False, default='just_simple_train',
